...
public class FileSystemTest extends TestCase {
	...
	public void testDuplicateName() throws DuplicateNameException {
		...
		try {
			...
			fail("Devia ter lan�ado DuplicateNameException");
		}
	...
	public void testDeepClone() throws DuplicateNameException {
		...
		assertEquals(crontab, crontab2);  // mas com o mesmo conte�do (nome, tamanho) 
	}
	...
	public void testMove() throws DuplicateNameException, CycleException {
		...
		// "move" permite alterar o pai e/ou o nome de um n�
		crontab.move(cron, "crontab2"); //newParent, newName
		assertEquals("/etc/cron/crontab2", crontab.getPath());
		
		// n�o se pode mover um n� para debaixo dele pr�prio ou de um descendente,
		// pois criaria um ciclo
		try {
			etc.move(cron, "etc");
			fail("Devia lan�ar CycleException");
		}
		catch(CycleException e) {			
		}

		// caso normal, s� para verificar que recuperou bem da excep��o anterior
		cron.move(root, "cron");
